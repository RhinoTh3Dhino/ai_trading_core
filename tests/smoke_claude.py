...
def check_summary():
    ...
    return "fallback" if txt.startswith("(AI slået fra)") else \
           "timeout"  if txt.startswith("(AI timeout/fejl)") else "ok"
...
def main():
    ...
    # health
    h = get("/healthz"); print("[✓] healthz:", h)
    # summary
    s = check_summary(); print(f"[✓] /ai/summary  => {s}")
    # explain
    e = check_explain(); print(f"[✓] /ai/explain  => {e}")
