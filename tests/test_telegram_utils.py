...
def test_telegram_enabled_false(monkeypatch):
    ...
    ), "telegram_enabled burde returnere False når variabler mangler"
    monkeypatch.setenv("TELEGRAM_TOKEN", "dummy_token")
    monkeypatch.setenv("TELEGRAM_CHAT_ID", "dummy_id")
    assert (
        not telegram_enabled()
    ), "telegram_enabled burde returnere False for dummy_token/dummy_id"
...
def test_telegram_enabled_true(monkeypatch):
    ...
    assert (
        telegram_enabled()
    ), "telegram_enabled burde returnere True med valide værdier"
...
def test_send_message_noop(monkeypatch, capsys):
    ...
    assert (
        "[TESTMODE]" in captured.out
    ), "Forventer [TESTMODE]-output når Telegram er deaktiveret"
...
def test_send_telegram_heartbeat(monkeypatch, capsys):
    ...
    assert (
        "botten kører stadig" in captured.out.lower()
    ), "Heartbeat besked mangler i output"
