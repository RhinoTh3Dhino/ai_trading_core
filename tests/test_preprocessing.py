...
def test_clean_and_normalize():
    ...
    assert not df_clean.isna().any().any(), f"Der er stadig NaN efter cleaning!\n{df_clean}"
    assert not np.isinf(df_clean.values).any(), f"Der er stadig inf efter cleaning!\n{df_clean}"
    assert df_clean['close'].max() < 1000, f"Outlier i 'close' er ikke fjernet!\n{df_clean}"
    ...
    assert abs(df_z["close_z"].mean()) < 1e-6, f"Mean for close_z er ikke ~0: {df_z['close_z'].mean()}"
    assert abs(df_z["volume_z"].mean()) < 1e-6, f"Mean for volume_z er ikke ~0: {df_z['volume_z'].mean()}"
