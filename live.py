        ...
        while True:
            try:
                ...
                if X_df.shape[1] != want_dim:
                    print(
                        f"[LIVE][warn] feature-dim={X_df.shape[1]} forventet={want_dim}. "
                ...
                last_prob = (
                    float(prob[-1])
                    if isinstance(prob, (list, np.ndarray, pd.Series))
                    else float(prob)
                )
                ...
                if "ema_200" in feats_all.columns:
                    regime = (
                        "trend"
                        if float(feats_all["close"].iloc[-1])
                        >= float(feats_all["ema_200"].iloc[-1])
                        else "meanrev"
                    )
                side_txt = "BUY" if last_sig == 1 else "SELL"

                print(
                    f"[LIVE] {bar_ts}  signal={last_sig} ({side_txt})  prob={last_prob:.3f}  price={price}  (features={X_df.shape[1]})"
                )
