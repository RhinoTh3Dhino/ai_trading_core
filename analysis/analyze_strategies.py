...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default=DEFAULT_OUTPUT_DIR, help="Output-mappe til alle filer (default=outputs/)")
...
def update_botstatus_md(metrics, run_id, plot_path, heatmap_path=None, md_path=None):
    with open(md_path, "a", encoding="utf-8") as f:
        ...
        for strat, vals in metrics.items():
            f.write(f"- {strat}: win-rate={vals['win_rate']:.1%}, profit={vals['profit']}, drawdown={vals['drawdown']}, trades={vals['trades']}\n")
...
def main():
    ...
    # Grafer
    plot_path = plot_strategy_scores(metrics, run_id, out_path=os.path.join(output_dir, f"strategy_scores_{run_id}.png"))
    heatmap_path = plot_strategy_history(csv_path=csv_path, out_path=os.path.join(output_dir, "strategy_score_heatmap.png"))
    # Telegram (valgfrit)
