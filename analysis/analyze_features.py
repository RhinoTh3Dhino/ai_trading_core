...
def save_feature_report(run_id, classic_png=None, permutation_png=None, shap_png=None, output_dir=OUTPUT_DIR):
...
def parse_args():
    ...
    parser.add_argument("--mode", type=str, choices=["classic", "permutation", "shap", "all"], default="all",
...
def main():
    ...
    if args.mode in ["classic", "all"]:
        feat_df, csv_path, classic_png = classic_feature_importance(model, X.columns, run_id, output_dir=OUTPUT_DIR)
    if args.mode in ["permutation", "all"]:
        feat_df2, csv_path2, permutation_png = permutation_feature_importance(model, X, y, run_id, output_dir=OUTPUT_DIR)
