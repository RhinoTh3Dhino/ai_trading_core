...
def ema_crossover_strategy(df: pd.DataFrame, fast=9, slow=21) -> pd.DataFrame:
    ...
    if not all(col in df.columns for col in [fast_col, slow_col]):
        if all(col in df.columns for col in ["ema_9", "ema_21"]):
            ...
            df['signal'] = 0
            return df
    df['signal'] = np.where(df[fast_col] > df[slow_col], 1,
                            np.where(df[fast_col] < df[slow_col], -1, 0))
    return df

def ema_rsi_regime_strategy(df: pd.DataFrame, rsi_entry=50, rsi_exit=45, regime_col="regime", regime_val="bull") -> pd.DataFrame:
    """
...
def ema_rsi_adx_strategy(df: pd.DataFrame, adx_threshold=20, regime_col="regime", regime_val="bull") -> pd.DataFrame:
    """
...
def regime_ensemble(df: pd.DataFrame, regime_col="regime") -> pd.DataFrame:
    ...
    df['signal'] = 0
    # Bull regime
    mask_bull = ((regime == "bull") | (regime == 1)) & (df['ema_9'] > df['ema_21']) & (df['rsi_14'] > 50)
    mask_bull_exit = ((regime == "bull") | (regime == 1)) & ((df['ema_9'] < df['ema_21']) | (df['rsi_14'] < 45))
...
def add_adaptive_sl_tp(df: pd.DataFrame, atr_col="atr_14", atr_mult_sl=1.5, atr_mult_tp=3.0) -> pd.DataFrame:
    """
