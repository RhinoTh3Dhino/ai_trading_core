...
def ema_crossover_strategy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all(col in df.columns for col in ["ema_9", "ema_21"]):
        df['signal'] = 0
        return df
    df['signal'] = np.where(df['ema_9'] > df['ema_21'], 1,
...
def ema_rsi_regime_strategy(df: pd.DataFrame, rsi_entry=50, rsi_exit=45) -> pd.DataFrame:
    ...
    Kun handel i bull regime (regime == 1).
    """
    df = df.copy()
    required_cols = ["ema_9", "ema_21", "rsi_14"]
    if not all(col in df.columns for col in required_cols):
        df['signal'] = 0
        return df
    regime = df['regime'] if 'regime' in df.columns else pd.Series(1, index=df.index)
    df['signal'] = 0
    buy_mask = (df['ema_9'] > df['ema_21']) & (df['rsi_14'] > rsi_entry) & (regime == 1)
    sell_mask = (df['ema_9'] < df['ema_21']) | (df['rsi_14'] < rsi_exit)
    df.loc[buy_mask, 'signal'] = 1
    df.loc[sell_mask, 'signal'] = -1
    df['signal'] = np.where(regime == 1, df['signal'], 0)  # Kun bull regime
    return df
...
def ema_rsi_adx_strategy(df: pd.DataFrame, adx_threshold: float = 20) -> pd.DataFrame:
    ...
    regime = df['regime'] if 'regime' in df.columns else pd.Series(1, index=df.index)
    df['signal'] = 0
    buy_mask = (df['ema_9'] > df['ema_21']) & (df['rsi_14'] > 50) & (adx > adx_threshold) & (regime == 1)
...
def regime_ensemble(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bull: EMA/RSI strategi
    Bear: Mean reversion strategi
    Neutral: Ingen handel
    """
    df = df.copy()
    required_cols = ["ema_9", "ema_21", "rsi_14"]
    if not all(col in df.columns for col in required_cols):
...
def voting_ensemble(df: pd.DataFrame) -> pd.DataFrame:
    ...
    strategies = [
        ...
        # ema_rsi_adx_strategy,  # Kan tilføjes for mere robusthed
    ]
...
# --- Bonus: Adaptive SL/TP baseret på ATR ---

def add_adaptive_sl_tp(df: pd.DataFrame, atr_col: str = "atr_14", atr_mult_sl=1.5, atr_mult_tp=3.0) -> pd.DataFrame:
    """
    ...
    if atr_col in df.columns and 'close' in df.columns:
        ...
        # Fald tilbage til standard værdier hvis ATR ikke findes
        df['sl_pct'] = 0.02
        df['tp_pct'] = 0.04
    return df
