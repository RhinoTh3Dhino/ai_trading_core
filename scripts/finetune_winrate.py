...
def main():
    parser = argparse.ArgumentParser(
        description="Gridsearch/finetune targets og features for winrate."
    )
    parser.add_argument(
        "--input",
        type=str,
        default=str(PROJECT_ROOT / "data" / "BTCUSDT_1h_features.csv"),
    ...
    for res in results[:10]:
        print(
            f"{res['target']} {res['features']} -> Winrate: {res['winrate']:.2%}, Accuracy: {res['accuracy']:.2%}, n_1: {res['n_pred_1']}, n_0: {res['n_pred_0']} ({res['n_test']})"
        )
    best = results[0]
