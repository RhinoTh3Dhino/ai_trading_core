...
def main():
    ...
    parser.add_argument("--input", type=str, default=str(PROJECT_ROOT / "data" / "BTCUSDT_1h_with_regime_target.csv"),
                        help="Sti til target-CSV med regime targets.")
    parser.add_argument("--target", type=str, default="target_regime_adapt", help="Target-kolonne (default: regime).")
    parser.add_argument("--features", type=str, default="close,rsi_14,ema_9,macd,macd_signal,vwap,atr_14,regime",
    ...
    parser.add_argument("--output", type=str, default=str(PROJECT_ROOT / "outputs" / "gridsearch_regime_results.csv"),
    ...
    for n_feats in range(1, args.max_features + 1):
        for feats in itertools.combinations(all_features, n_feats):
            try:
                ...
                print(f"Features: {feats} | Accuracy: {acc:.3f} | Winrate: {winrate:.3f} | Sharpe: {sharpe:.2f} | n_1: {n_1}, n_0: {n_0}, n_test: {n_test}")
    ...
    if not results_df.empty:
        ...
        msg = (f"📊 Gridsearch Regime-Target\n"
