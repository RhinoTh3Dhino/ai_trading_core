...
metrics = [
    ...
    "test_rolling_sharpe",
    "test_trade_duration",
    "test_regime_drawdown",
]
print("\n--- Samlet statistik for alle splits ---")
for m in metrics:
    if m in df.columns:
        print(
            f"{m:<20} | Mean: {df[m].mean():>8.2f} | Min: {df[m].min():>8.2f} | Max: {df[m].max():>8.2f}"
        )
...
sharpe_plot = (
    fPROJECT_ROOT
    / "outputs"
    / "walkforward/plot_sharpe_per_split_{datetime.now():%Y%m%d_%H%M%S}.png"
)
...
winrate_plot = (
    fPROJECT_ROOT
    / "outputs"
    / "walkforward/plot_winrate_buyhold_per_split_{datetime.now():%Y%m%d_%H%M%S}.png"
)
...
if "test_rolling_sharpe" in df.columns:
    ...
    for symbol in df["symbol"].unique():
        ...
        plt.plot(
            sub["window_start"],
            sub["test_rolling_sharpe"],
            label=f"{symbol} Rolling Sharpe",
        )
    plt.xlabel("Walkforward Window Start")
    ...
    fname = (
        fPROJECT_ROOT
        / "outputs"
        / "walkforward/plot_rolling_sharpe_{datetime.now():%Y%m%d_%H%M%S}.png"
    )
    plt.savefig(fname)
...
if "test_trade_duration" in df.columns:
    ...
    for symbol in df["symbol"].unique():
        ...
        plt.plot(
            sub["window_start"],
            sub["test_trade_duration"],
            label=f"{symbol} Trade Duration",
        )
    plt.xlabel("Walkforward Window Start")
    ...
    fname = (
        fPROJECT_ROOT
        / "outputs"
        / "walkforward/plot_trade_duration_{datetime.now():%Y%m%d_%H%M%S}.png"
    )
    plt.savefig(fname)
    print(f"✅ Gemte plot: {fname}")
...
if "test_regime_drawdown" in df.columns:
    ...
    for symbol in df["symbol"].unique():
        ...
        plt.plot(
            sub["window_start"],
            sub["test_regime_drawdown"],
            label=f"{symbol} Regime DD",
        )
    plt.xlabel("Walkforward Window Start")
    ...
    fname = (
        fPROJECT_ROOT
        / "outputs"
        / "walkforward/plot_regime_drawdown_{datetime.now():%Y%m%d_%H%M%S}.png"
    )
    plt.savefig(fname)
    print(f"✅ Gemte plot: {fname}")
...
for N, tag in [(5, "top5"), (10, "top10")]:
    ...
    topN_csv = (
        fPROJECT_ROOT
        / "outputs"
        / "walkforward/{tag}_sharpe_splits_{datetime.now():%Y%m%d_%H%M%S}.csv"
    )
...
if "symbol" in df.columns and "timeframe" in df.columns and "test_sharpe" in df.columns:
    pivot = pd.pivot_table(
        df, values="test_sharpe", index="symbol", columns="timeframe", aggfunc="mean"
    )
