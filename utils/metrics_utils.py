...
def max_consecutive_losses(trades_df):
    ...
    return (
        losses.groupby((losses != losses.shift()).cumsum()).cumsum() * losses
    ).max() or 0
...
def recovery_bars(balance_df):
    ...
    if dd_end is not None and not np.isnan(dd_end):
        ...
        recovery_idx = balance_after_dd[
            balance_after_dd >= prev_peak
        ].first_valid_index()
        if recovery_idx is not None:
            return balance_df.index.get_loc(recovery_idx) - balance_df.index.get_loc(
                dd_end
            )
    return -1
...
def advanced_performance_metrics(trades_df, balance_df, initial_balance=1000):
    ...
    profit_pct = (
        (balance_df["balance"].iloc[-1] - initial_balance) / initial_balance * 100
    )
    peak = balance_df["balance"].cummax()
    drawdown = (balance_df["balance"] - peak) / peak
