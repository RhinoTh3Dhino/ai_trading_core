...
def log_top_features_csv(top_features, csv_path="data/top_features_history.csv", model_name="ML", run_time=None):
...
def send_top_features_telegram(top_features, send_telegram_message, chat_id, model_name="ML"):
    msg = f"📊 Top-5 features ({model_name}):\n" + \
          "\n".join([f"{i+1}. {name}: {score:.4f}" for i, (name, score) in enumerate(top_features)])
