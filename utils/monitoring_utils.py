...
def send_live_metrics(
    ...
    if alert_on_drawdown and check_drawdown_alert(metrics, thresholds.get("drawdown")):
        alarm_msgs.append(f"‼️ ADVARSEL: Drawdown ({metrics['drawdown_pct']:.2f}%) under grænsen ({thresholds.get('drawdown', '-')}%)!")
    if alert_on_winrate and check_winrate_alert(metrics, thresholds.get("winrate")):
        alarm_msgs.append(f"‼️ ADVARSEL: Win-rate ({metrics['win_rate']:.1f}%) under grænsen ({thresholds.get('winrate', '-')}%)!")
    if alert_on_profit and check_profit_alert(metrics, thresholds.get("profit")):
        alarm_msgs.append(f"‼️ ADVARSEL: Profit ({metrics['profit_pct']:.2f}%) under grænsen ({thresholds.get('profit', '-')}%)!")
