...
def generate_target(df: pd.DataFrame, tp_multiplier=1.5, sl_multiplier=1.0, lookahead=12, target_col=None):
...
def main():
    parser = argparse.ArgumentParser(description="Generér target-kolonner til AI trading pipeline – flere TP/SL på én gang!")
    parser.add_argument("--input", type=str, default=str(PROJECT_ROOT / "data" / "BTCUSDT_1h.csv"))
    parser.add_argument("--output", type=str, default=str(PROJECT_ROOT / "data" / "BTCUSDT_1h_with_target.csv"))
    parser.add_argument("--tp", type=str, default="1.5", help="Kommasepareret TP-multipler (fx '1.0,1.5,2.0')")
    parser.add_argument("--sl", type=str, default="1.0", help="Kommasepareret SL-multipler (fx '1.0,1.5')")
    ...
    for tp in tp_list:
        for sl in sl_list:
            ...
            series = generate_target(df_orig, tp_multiplier=tp, sl_multiplier=sl, lookahead=args.lookahead, target_col=colname)
