...
REQ_COLS = [
    ...
    "macd","macd_signal","macd_hist","rsi_14","rsi_28","atr_14",
    "bb_upper","bb_lower","vwap","zscore_20","return","pv_ratio","regime",
    "rsi_28_z","regime_z","macd_z","ema_200_z","rsi_14_z","ema_9_z","vwap_z",
    "zscore_20_z","bb_upper_z","bb_lower_z","ema_50_z","atr_14_z","ema_21_z",
]
...
def _z(s):
    s = pd.to_numeric(s, errors="coerce").astype(float)
    std = s.std(ddof=0)
    if not np.isfinite(std) or std==0: return pd.Series(0.0, index=s.index)
    return (s - s.mean())/std
...
def compute_features(ohlcv: pd.DataFrame) -> pd.DataFrame:
    ...
    df["atr_14"] = (atr.replace([0,np.inf,-np.inf],np.nan)
                      .bfill().ffill().fillna(1e-6).clip(lower=1e-6))
    # Bollinger 20,2
    ...
    # regime dummy
    if "regime" not in df: df["regime"] = 0
    # _z cols
    for base in ["rsi_28","regime","macd","ema_200","rsi_14","ema_9","vwap","zscore_20",
                 "bb_upper","bb_lower","ema_50","atr_14","ema_21"]:
        zname = f"{base}_z"
        if zname not in df:
            col = df[base] if base in df else pd.Series(0.0, index=df.index)
            if col.dtype=="O": col = pd.Series(pd.factorize(col)[0], index=col.index, dtype=float)
            df[zname] = _z(col)
    # sørg for alle REQ_COLS
    for ccol in REQ_COLS:
        if ccol not in df: df[ccol] = 0.0
    return df
