...
def log_device_status(data_path, batch_size, epochs, lr):
    ...
    status_line = (f"{now} | TensorFlow {tf_version} | Python {python_version} | "
...
def train_keras_model(
    ...
    tensorboard_callback = keras.callbacks.TensorBoard(log_dir=f"runs/{tb_run_name}", histogram_freq=1)
    ...
    # Byg model
    model = build_model(input_dim=X.shape[1], n_classes=n_classes, hidden_dim=hidden_dim, n_layers=n_layers, dropout=dropout)
    ...
    if save_model:
        callbacks.append(
            keras.callbacks.ModelCheckpoint(
                MODEL_PATH, monitor=monitor, save_best_only=True, save_weights_only=False, verbose=1
            )
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Træn Keras/TensorFlow-model til trading + MLflow, TensorBoard, mixed precision")
    ...
    parser.add_argument("--monitor", type=str, default="val_loss", choices=["val_loss", "val_accuracy"], help="Monitor for early stopping")
    parser.add_argument("--min_delta", type=float, default=1e-4, help="Minimum forbedring (delta) før early stopping resetter")
    parser.add_argument("--mixed_precision", action="store_true", help="Aktiver mixed precision (float16 training)")
