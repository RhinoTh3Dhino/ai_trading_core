...
def train_lstm_model(df, feature_cols, target_col="target", seq_length=48, epochs=30, batch_size=64, model_path=PROJECT_ROOT / "models" / "lstm_model.h5"  # AUTO PATH CONVERTED):
    ...
    model = build_lstm_model(input_shape=(seq_length, len(feature_cols)), n_classes=len(np.unique(y)))
    callbacks = [
...
if __name__ == "__main__":
    ...
    feature_cols = [col for col in num_cols if col not in [args.target, "future_return", "timestamp"]]
    ...
    if os.path.exists(feature_path):
        ...
        if feature_cols != prev_features:
            print(f"[ADVARSEL] Dine features matcher IKKE tidligere gemt feature-liste!\nTidligere: {prev_features}\nNye: {feature_cols}")
    ...
    print(f"Træning færdig. Model gemt til '{args.model_out}'. Features og scaler gemt i 'models/'.")